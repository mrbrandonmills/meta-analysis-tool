...
async def run_workflow_in_background(analysis_id: str):
    """Run the complete meta-analysis workflow in the background.
    ...
    try:
        ...
        async with async_session() as db:
            ...
            if not meta_analysis:
                logger.error(f"Meta-analysis not found in background: {analysis_id}")
                return
            ...
            if not coordinator:
                logger.error(f"Coordinator not found for {analysis_id} in background workflow")
                await service.update_meta_analysis_status(analysis_uuid, MetaAnalysisStatus.FAILED)
                await db.commit()
                return
            ...
            logger.info(f"Running SearchAgent for {analysis_id}")
            ...
            logger.info(f"Running ScreeningAgent for {analysis_id}")
            ...
            logger.info(f"Running CredibilityAgent for {analysis_id}")
            ...
            await service.update_meta_analysis_status(analysis_uuid, MetaAnalysisStatus.COMPLETED)
            await db.commit()

            logger.info(f"Background workflow completed successfully for {analysis_id}")
...
async def execute_meta_analysis(
    ...
    """Execute a meta-analysis workflow in the background.

    This endpoint:
    1. Validates the meta-analysis exists
    2. Initializes the coordinator (if first execution)
    3. Updates status to IN_PROGRESS
    4. Starts the workflow in background
    5. Returns immediately with status

    The actual workflow runs asynchronously. Use the status endpoint
    to check progress.
    """
    try:
        ...
        if meta_analysis.status == MetaAnalysisStatus.IN_PROGRESS:
            return {
                "analysis_id": analysis_id,
                "status": "in_progress",
                "message": "Workflow already running. Check status endpoint for progress.",
            }
        ...
        if meta_analysis.status == MetaAnalysisStatus.COMPLETED:
            return {
                "analysis_id": analysis_id,
                "status": "completed",
                "message": "Workflow already completed. Use status endpoint for results.",
            }
        ...
        background_tasks.add_task(run_workflow_in_background, analysis_id)

        logger.info(f"Workflow started in background for {analysis_id}")

        return {
            "analysis_id": analysis_id,
            "status": "processing",
            "message": "Meta-analysis workflow started in background. Use the status endpoint to check progress.",
            "status_endpoint": f"/api/v1/meta-analysis/status/{analysis_id}",
        }
...
async def get_status(analysis_id: str, db: AsyncSession = Depends(get_async_db)):
    """Get the detailed status of a meta-analysis including agent execution progress."""
    try:
        ...
        completed_agents = len([e for e in agent_executions if e.status == "success"])
        total_expected_agents = 3  # Search, Screening, Credibility

        progress_percentage = (completed_agents / total_expected_agents * 100) if total_expected_agents > 0 else 0
        ...
        return {
            ...
            "agents_total": total_expected_agents,
        }
